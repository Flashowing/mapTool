...
<!--    </div>-->
    ...
    <van-action-sheet v-model='info.infoShow' :title='info.infoTitle'>
      <div class='content'>
        <table v-for='(value, key) in info.infoContent'>
          <tr>
            <td>{{ key }}</td>
            <td>{{ value }}</td>
          </tr>
        </table>
        <van-button type='primary' @click='gather'>采集</van-button>
      </div>
    </van-action-sheet>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    switchRoadStreet() {
      this.roadProvider.show = !this.roadProvider.show
      this.roadProvider.show ? this.btnStatus = 'info' : this.btnStatus = 'default'
    },
  ...
  mounted: function() {
    ...
    this.roadProvider = viewer.imageryLayers.addImageryProvider(new Cesium.WebMapTileServiceImageryProvider({
      // 影像注记
    ...
    function createDescription(feature) {
      ...
      html += `</table>
<!--  <van-button style='text-align: center;' type='primary'>主要按钮</van-button>-->
</div>`
      return html
    }
    ...
    handler.setInputAction(function(event) {
      ...
      if (pick.id.customProp) {
        // _this.info.infoShow = true
        _this.info.infoTitle = pick.id.customProp.name
        _this.info.infoContent = pick.id.customProp
      }
